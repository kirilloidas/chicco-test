<template>
  <section class="products-section">

    <h2 class="products-section__title">
      Popular Products
    </h2>

    <GSCarousel
      :items="items.concat(...items)"
      item-gap="16"
      :items-to-show="1"
      :layout="CustomLayout"
      class="products-section__carousel"
    >
      <template #item="{ data }">
        <CarouselCard
          :item="data"
        />
      </template>
    </GSCarousel>

    <button class="products-section__btn">
      Explore all items
      <svg width="21" height="13" viewBox="0 0 21 13" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path fill-rule="evenodd" clip-rule="evenodd" d="M13.5555 12.7274C13.469 12.6411 13.4004 12.5387 13.3536 12.4259C13.3068 12.3131 13.2827 12.1922 13.2827 12.0701C13.2827 11.948 13.3068 11.8271 13.3536 11.7143C13.4004 11.6015 13.469 11.499 13.5555 11.4128L18.4702 6.49988L13.5555 1.58695C13.3811 1.41263 13.2832 1.17619 13.2832 0.929664C13.2832 0.683134 13.3811 0.446701 13.5555 0.272378C13.7298 0.0980549 13.9662 0.00012204 14.2127 0.00012203C14.4593 0.000122019 14.6957 0.0980548 14.87 0.272378L20.4402 5.84259C20.5267 5.92883 20.5953 6.03128 20.6421 6.14407C20.6889 6.25685 20.713 6.37777 20.713 6.49988C20.713 6.62199 20.6889 6.7429 20.6421 6.85569C20.5953 6.96848 20.5267 7.07093 20.4402 7.15716L14.87 12.7274C14.7838 12.8138 14.6813 12.8824 14.5686 12.9292C14.4558 12.976 14.3349 13.0001 14.2127 13.0001C14.0906 13.0001 13.9697 12.976 13.8569 12.9292C13.7441 12.8824 13.6417 12.8138 13.5555 12.7274Z" fill="white"/>
        <path fill-rule="evenodd" clip-rule="evenodd" d="M0.287109 6.49998C0.287109 6.25376 0.38492 6.01762 0.559023 5.84352C0.733126 5.66942 0.96926 5.57161 1.21548 5.57161L17.9261 5.57161C18.1723 5.57161 18.4085 5.66941 18.5826 5.84352C18.7567 6.01762 18.8545 6.25376 18.8545 6.49997C18.8545 6.74619 18.7567 6.98233 18.5826 7.15643C18.4085 7.33053 18.1723 7.42834 17.9261 7.42834L1.21548 7.42834C0.96926 7.42834 0.733126 7.33053 0.559023 7.15643C0.38492 6.98233 0.287109 6.74619 0.287109 6.49998Z" fill="white"/>
      </svg>
    </button>
  </section>
</template>

<script setup>
import { GSCarousel } from 'gitart-scroll-carousel'
import CustomLayout from './components/CustomLayout.vue';
import CarouselCard from './components/CarouselCard.vue';

const getImage = imgName => new URL(`/src/assets/img/${imgName}`, import.meta.url)
const items = [
  {
    cardImg: getImage('armchair.png'),
    cardTitle: 'Armchair',
    cardDescription: 'Light single chair',
    cardPrice: '$145',
    cardBgColor: '#CAF3E5',
  },
  {
    cardImg: getImage('premium-sofa.png'),
    cardTitle: 'Premium Sofa',
    cardDescription: 'Light single chair',
    cardPrice: '$145',
    cardBgColor: '#E0EFF6',
  },  {
    cardImg: getImage('minimal-sofa.png'),
    cardTitle: 'Minimal Sofa',
    cardDescription: 'Light single chair',
    cardPrice: '$145',
    cardBgColor: '#EEEBFF',
  },
  {
    cardImg: getImage('dining-chair.png'),
    cardTitle: 'Dining Chair',
    cardDescription: 'Light single chair',
    cardPrice: '$145',
    cardBgColor: '#FFF4E7',
  }
]
</script>

<style lang="scss" scoped>

.products-section {
  background-image: url('@/assets/img/chair-bg.png');
  background-repeat: no-repeat;
  background-position: 0 0;
  display: flex;
  flex-direction: column;
  align-items: center;
  background-size: contain;

  &__title {
    margin-top: 120px;
    margin-bottom: 58px;
    font-family: 'Playfair Display';
    font-weight: 700;
    font-size: 64px;
    line-height: 85px;
  }

  &__carousel {
    width: 100%;

    :deep(.gsc-track__item) {
      width: auto !important;
    }
  }

  &__btn {
    margin-top: 54px;
    margin-bottom: 113px;
    background: #70908B;
    border-radius: 10px;
    padding: 24px 38px;
    display: flex;
    align-items: center;
    justify-content: space-between;
    font-family: 'Open Sans';
    font-weight: 700;
    font-size: 18px;
    line-height: 25px;
    color: #FFFFFF;
    gap: 0 18px;
  }
}

@media (max-width: 900px) {
  .products-section {
    &__title {
      margin-top: 53px;
      margin-bottom: 104px;
    }
    &__btn {
      margin-bottom: 100px;
    }
  }
}

@media (max-width: 600px) {
  .products-section {
    &__title {
      font-size: 36px;
      line-height: 52px;
      margin-top: 42px;
      margin-bottom: 60px;
    }
    &__btn {
      margin-bottom: 80px;
    }
  }
}

</style>